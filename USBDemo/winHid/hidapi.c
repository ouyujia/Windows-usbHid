...
static HANDLE open_device(const wchar_t *path, BOOL open_rw)
{
    ...
    DWORD share_mode = FILE_SHARE_READ|FILE_SHARE_WRITE;//FILE_SHARE_READ|FILE_SHARE_WRITE;//0
    //独占读写。不然可以和校准工具同时连接，数据混乱。
...
static void hid_internal_get_info(const wchar_t* interface_path, struct hid_device_info* dev)
{
    ...
    for (hardware_id = hardware_ids; *hardware_id; hardware_id += wcslen(hardware_id) + 1) {
		/* Normalize to upper case */
        wchar_t* p;
        for (p = hardware_id; *p; ++p) *p = towupper(*p);
    ...
    for (compatible_id = compatible_ids; *compatible_id; compatible_id += wcslen(compatible_id) + 1) {
		/* Normalize to upper case */
        wchar_t* p;
        for (p = compatible_id; *p; ++p) *p = towupper(*p);
...
struct hid_device_info HID_API_EXPORT * HID_API_CALL hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
    ...
    for (device_interface = device_interface_list; *device_interface; device_interface += wcslen(device_interface) + 1) {
...
int  HID_API_EXPORT HID_API_CALL hid_getInterfaceNO(hid_device *dev)
{
    return dev->device_info->interface_number;
}
